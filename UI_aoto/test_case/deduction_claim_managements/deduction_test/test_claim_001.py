...
class Test_01(unit_star_end.Test_define):
    ...
    '''在贷客户进行认领操作'''
    def test_01_claim_index(self):
        '''验证新进订单，进行贷前(正常)认领操作'''
        self.logs.info_log(u'验证新进订单，进行贷前（正常认领）认领操作；')
        ...
        self.driver.element(self.browser, 'name', loan_clients.number_input_name).send_keys(self.customerName)
        # 获取订单id
