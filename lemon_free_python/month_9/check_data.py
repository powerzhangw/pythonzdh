...
for item in tencent_data:
    for value in our_data:
        if item[1]==value[1] and item[2]==value[2]:#qq和两者都相等 就可以称之为核对正常数据
            #更新腾讯的数据
            if tencent_sheet.cell(item[0],9).value and tencent_sheet.cell(item[0],10).value!=None:
                tencent_sheet.cell(item[0],11).value='异常数据'
            else:
